...
async function phase4Demo() {
    ...
    const data1 = await heatmap1.json();
    const reports = data1.filter((d) => d.type === 'REPORT');
    const scans = data1.filter((d) => d.type === 'SCAN');
    ...
    const data2 = await heatmap2.json();
    const reports2 = data2.filter((d) => d.type === 'REPORT');
    const scans2 = data2.filter((d) => d.type === 'SCAN');
