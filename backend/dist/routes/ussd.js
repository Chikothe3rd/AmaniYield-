...
router.post('/', async (req, res) => {
    ...
    let user = await server_1.prisma.user.findUnique({ where: { phoneNumber } });
...
2. No`;
            ...
            try {
                const coords = geo_1.REGION_COORDINATES[user.region] || geo_1.REGION_COORDINATES['Lusaka'];
                await server_1.prisma.uSSD_Report.create({
                    data: {
                        resourceType: 'WATER',
                        status: 'DRY',
                        latitude: coords.lat,
        ...
        try {
            const userCoords = user ? (geo_1.REGION_COORDINATES[user.region] || geo_1.REGION_COORDINATES['Lusaka']) : geo_1.REGION_COORDINATES['Lusaka'];
            const activeRoutes = await server_1.prisma.safe_Route.findMany({
                where: { status: 'ACTIVE' }
            });
            if (activeRoutes.length > 0) {
                // Find the nearest route
                let nearestRoute = activeRoutes[0];
                let minDistance = (0, geo_1.calculateDistance)(userCoords.lat, userCoords.lng, nearestRoute.destination_lat, nearestRoute.destination_lng);
                for (const route of activeRoutes) {
                    const dist = (0, geo_1.calculateDistance)(userCoords.lat, userCoords.lng, route.destination_lat, route.destination_lng);
                    if (dist < minDistance) {
                        minDistance = dist;
                        nearestRoute = route;
                    }
                ...
                await (0, sms_1.sendSMS)(phoneNumber, smsMessage);
            }
            else {
        ...
        try {
            ...
            const advice = await (0, weather_1.getPlantingAdvice)(region);
